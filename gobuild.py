...
def main():
    if len(sys.argv) != 4:
        ...
        sys.exit(1)
    out_path = os.path.join(os.getcwd(), sys.argv[1])
    ...
    if os.environ.get('CI') == 'true':
        buildflags.append('-ldflags=-s -w -extldflags -Wl,--unresolved-symbols=ignore-all')
    subprocess.run(
        ['go', 'build'] + buildflags + ['.'],
        check=True,
        cwd=os.path.dirname(__file__),
        env=dict(os.environ, CGO_CFLAGS=shlex.join(cflags), CGO_LDFLAGS='-Wl,--unresolved-symbols=ignore-all'),
    )
